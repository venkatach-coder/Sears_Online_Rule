...
def core_rule(row):
    if row['min_comp'] is not None:
        if row['min_comp_MM'] is not None:
            if 389.0 <=row['min_comp_MM']<= 399.0:
                return 399.0
            else:
                return row['min_comp_MM'], 'Match at Min_Comp_MM'
        else:
            if 389.0 <= row['min_margin'] <= 399.0:
                return 399.0
            else:
...
def construct_rule(rule_target_sql_str, rule_level, *args, **kwargs) -> dp_rules.DP_Rule:
    ...
    thisrule.add_rule_layer(
        thisrule.core_rule_wrapper(core_rule, 'min_comp_MM, min_margin, 399')
    )
    thisrule.add_rule_layer(
        thisrule.uplift_wrapper(default_rules.default_uplift, 'additional uplift')
    )
    thisrule.add_rule_layer(
