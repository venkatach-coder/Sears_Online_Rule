...
class Construct_DP_Rule(harlem.DP_Rule_Constructor):
    def __init__(self):
        super().__init__(rule_level=9001,
                         scope='*',
                         is_active = True, rule_name='BASE PMI rule')
    ...
    def get_core_rule(self):
        func_handle = partial(core_rule._Thresh_Set_to_PMI_when_PMI_exists, thresh=14.99)
        return [
            Working_func(func_handle, 'Send PMI when PMI <= 14.99')
        ]


    def get_uplift_rule(self):
        func_handle = partial(uplift_rule._uplift_by_percentage_threshhold, thresh=14.99, uplift=1.4,
                              max_val=float('inf'))
        return [
