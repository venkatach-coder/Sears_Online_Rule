...
class Construct_DP_Rule(harlem.DP_Rule_Constructor):
    def __init__(self):
        super().__init__(rule_level=1000,
                         scope='div_no = 26 and ln_no in (1,2,4)', rule_name='div26 HA DP Rule', )
    ...
    def get_min_margin_func(self):
        def min_margin_rule(row):
            ...
            if row['ln_no'] != 4 and (re.match('samsung', brand.lower()) is None) and (re.match('samsung', product_brand.lower()) is None):
                return round(row['cost_with_subsidy'] / 0.85, 2), '0.15'
            if row['ln_no'] != 4 and ((re.match('samsung', brand.lower()) is not None) or (re.match('samsung', product_brand.lower()) is not None)):
