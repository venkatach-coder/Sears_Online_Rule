...
class Construct_DP_Rule(harlem.DP_Rule_Constructor):
    def __init__(self):
        super().__init__(rule_level=500,
                         scope='div_no in (22, 26, 20, 46)',
                         is_active = False,
                         rule_name='pmi kenmore 22 26 20 46rule')
    ...
    def get_uplift_rule(self):
        func_handle = partial(uplift_rule._uplift_by_percentage_max, uplift=1.00, max_val = float('inf'))
        return [
            Working_func(func_handle, '1.00 kenmore')
        ]
