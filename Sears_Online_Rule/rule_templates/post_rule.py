...
def _Min_Reg_PMI_Upliftted_Prcie_D_flag(row):
    if row['ad_plan'] == 'D':
        ...
        dp_price = row.uplift_rule.value if row.uplift_rule.value is not None else float('inf')
...
def _Reg_Bound_check_Null_when_reg_not_Exists(row):
    ...
    if row.uplift_rule.value <= row['reg']:
        return row.uplift_rule.value, 'Set Price to DP_price'
    else:
