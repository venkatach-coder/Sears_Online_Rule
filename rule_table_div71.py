...
def core_rule(row):
    ...
    if row['min_comp'] is not None:
        ...
        if row['PMI'] is not None:
            return row['PMI'] * 0.99, 'PMI* 0.99' if 1 - row['cost_with_subsit'] / row['PMI'] >= 0.3 else \
                row['PMI']*1.02, 'PMI'
...
def construct_rule(rule_target_sql_str, rule_level) -> dp_rules.DP_Rule:
    ...
    thisrule.add_rule_layer(
        thisrule.core_rule_wrapper(core_rule, 'min_comp_MM, min_margin, PMI(HIGH, LOW)')
    )
    thisrule.add_rule_layer(
        thisrule.uplift_wrapper(default_rules.default_uplift, 'Default uplift')
    )
    thisrule.add_rule_layer(
        thisrule.post_rule_wrapper(post_rule, 'Custom postrule')
    )
