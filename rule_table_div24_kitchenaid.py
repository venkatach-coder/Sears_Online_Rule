...
def core_rule(row):
    if row['MAP_price'] is not None: return row['MAP_price']
    if row['PMI'] is not None: return row['PMI']
...
def construct_rule(rule_target_sql_str, rule_level, *args, **kwargs) -> dp_rules.DP_Rule:
    thisrule = dp_rules.DP_Rule(
        ...
        rule_name='div24 kitchenaid rule',
        if_exists='append',
    ...
    thisrule.add_rule_layer(
        thisrule.uplift_wrapper(home_uplift.uplift, 'additional uplift')
    )
    thisrule.add_rule_layer(
        thisrule.post_rule_wrapper(default_rules.default_postrule, 'VD MAP post_rule')
    )
    return thisrule
